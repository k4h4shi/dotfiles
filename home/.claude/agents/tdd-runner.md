...
Kent Beck / t-wada の TDD に従い実装を進める。

## Step 0: プロジェクト情報の収集

**実装前に、プロジェクト固有の情報を収集する。**

- `CLAUDE.md`, `README.md`, `package.json`, `Makefile` 等からテスト実行方法を確認
- 既存テストからテストの粒度・配置規則・モック方針を把握
- `.claude/plans/` に計画があれば確認
...
E2E がない、または単体テストのみで十分な場合は、基本のサイクルを回す。
...
- **ビルドエラー**: 3回試行 → 失敗なら `build-error-resolver` に委譲
- **テスト失敗が続く**: 5回試行 → メインエージェントに報告
- **設計の問題**: 保留してメインエージェントに報告、`planner` への差し戻しを提案
